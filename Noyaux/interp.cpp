...
int get_nearest_key(std::vector<double> &arr, double value)
{
    auto it = std::lower_bound(arr.begin(), arr.end(), value);
    if(it == arr.end()) return -1;
    if(it == arr.end()-1) return it-arr.begin()-1;
    double ld = abs(value-*(it-1));
    double ud = abs(value-*(it));
    return (ld > ud) ? it-arr.begin() : it-arr.begin()-1;
}

double element_abundance(abundance_table &table, int A, int Z, double T, double nb, double Y)
{
    int T_key = get_nearest_key(table.parameters[0], T);
    int nb_key = get_nearest_key(table.parameters[1], nb);
    int Y_key = get_nearest_key(table.parameters[2], Y);

    printf("%d %d %d\n", T_key, nb_key, Y_key);
 
    std::array<int, 3> lower = { T_key, nb_key, Y_key };
    std::array<int, 3> next[3] = { { T_key+1, nb_key, Y_key }, { T_key, nb_key+1, Y_key }, { T_key, nb_key, Y_key+1 } };
...
int main(int argc, char *argv[])
{
    ...
    FILE *fp = fopen("out2.res", "w+");
    std::array<int, 3> conditions = { 36,87,27 };
    abundance_data ab = *table.abundances[conditions];
